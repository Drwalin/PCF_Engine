...
{
	printf( "\n    Updating Terrain Colliders" );
	float xmin = triPHYS[0].vertex[0][0], zmin = triPHYS[0].vertex[0][2];
	float xmax = triPHYS[0].vertex[0][0], zmax = triPHYS[0].vertex[0][2];
	unsigned int i, j;
	for( i = 1; i < triPHYS.size(); i++ )	// looking for lower and heigher possition of triangles
	{
	...
	if( colliderter != NULL )
	{
		ColliderTerrain * ttttttt____ = &(colliderter[0][0]);
		printf( "\n     Terrain Colliders destroying" );
		for( i = 0; i < maximumnumberoftrianglescolliders[0]; ++i )
		{
//			if( *(colliderter+i) != NULL )
			{
				for( j = 0; j < maximumnumberoftrianglescolliders[1]; ++j )
				{
					colliderter[i][j].tri.resize( 0 );
//					((*(colliderter+i))+j) -> tri.resize( 0 );
				}
	...
	if( colliderobj != NULL )
	{
		printf( "\n     Objects Colliders destroying" );
		for( i = 0; i < maximumnumberofobjectcolliders[0]; ++i )
		{
//			if( *(colliderobj+i) != NULL )
			{
				for( j = 0; j < maximumnumberofobjectcolliders[1]; ++j )
				{
					colliderobj[i][j].staticmodel.resize( 0 );
//					((*(colliderobj+i))+j) -> staticmodel.resize( 0 );
				}
...
inline void Map::UpdateObjectColliders()
{
	...
	colliderobjectout.staticmodel.resize( 0 );
