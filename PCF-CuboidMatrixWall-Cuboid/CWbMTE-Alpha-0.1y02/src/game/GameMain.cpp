...
//	ShowCursor( false );
    ...
    game.engine.map.vbotex[0].SetTexture( "Media\\textmodel.bmpx" );
	/*
    	load textures and models:
    */
    game.engine.AddGraphicModel( "testmap.map" );
    game.engine.GraphicModels[0].Init( "Media\\texture1.bmpx" );
	...
	game.engine.map.staticobject.resize( 1000 );
	for( int i = 0; i < game.engine.map.staticobject.size(); i++ )
	{
		...
		game.engine.map.staticobject[i].pos.Set( (float(i/45)*2.0f) - 48.0f, 60.0f+float(i)*0.01f, (float(i%45)*2.0f) - 48.0f );
