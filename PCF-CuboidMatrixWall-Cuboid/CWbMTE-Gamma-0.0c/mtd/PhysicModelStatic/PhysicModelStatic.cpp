...
inline Vector PhysicModelStatic::MaxWithVector( Vector dst )
{
	...
	if( dst[0] < 0.0f )			point[0] = pos[0];
	else if( dst[0] > 0.0f )	point[0] = pos[0] + size[0];
	if( dst[1] < 0.0f )			point[1] = pos[1];
	else if( dst[1] > 0.0f )	point[1] = pos[1] + size[1];
	if( dst[2] < 0.0f )			point[2] = pos[2];
	else if( dst[2] > 0.0f )	point[2] = pos[2] + size[2];
	return point;
}
