...
{
//	if( abs( normal.Dot( p )+det ) <= 0.001f  )
		if( wallnormals[0].Dot( p )+walldet[0] >= -0.001f )
			if( wallnormals[1].Dot( p )+walldet[1] >= -0.001f )
				if( wallnormals[2].Dot( p )+walldet[2] >= -0.001f )
					return true;
	return false;
}
...
inline bool TrianglePX::PointIn( Vector p1, Vector p2 )
{
	...
	float t = ( - det - normal.Dot(p1) ) / dot;//( - det - normal.Dot( p1 ) ) / dot;
...
{
	Vector versor = (p2-p1).Versor(), fast;
	float dot = normal.Dot( versor );
	if( dot == 0.0f )
	{
		return false;
	...
	if( t > (p2-p1).Lenght() || t < 0.0f )
	{
		return false;
	}
	fast = p1 + (versor*t);
	if( PointIn( fast ) == true )
	{
		*dst = fast;
		return true;
	}
...
inline Vector TrianglePX::GetCrossPoint( Vector p1, Vector p2 )
{
	Vector versor = (p2-p1).Versor();
	float dot = normal.Dot( versor );
	if( dot == 0.0f )	return SetVector( 0.0f, 0.0f, 0.0f );
	float t = ( -det-(normal.Dot(p1))) / dot;
	if( t > (p2-p1).Lenght() || t < 0.0f )	return SetVector( 0.0f, 0.0f, 0.0f );
	return p1 + (versor*t);
}
