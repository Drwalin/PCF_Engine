...
inline void Map::UpdateObjectsCollisions( float FrameTime, int begin, int end )
{
    ...
    for( i = begin; i < staticobject.size() && i < end; ++i )
    {
		...
		{
//			printf( "\n   |||%i|||", staticobject[i] -> MainVectorId );
			
			x_min = ( staticobject[i]->pos[0] - moveColliderTerrain[0] ) / sizeColliderTerrain[0];
			x_max = ( staticobject[i]->pos[0] + staticobject[i]->size[0] - moveColliderTerrain[0] ) / sizeColliderTerrain[0];
			z_min = ( staticobject[i]->pos[2] - moveColliderTerrain[1] ) / sizeColliderTerrain[1];
			z_max = ( staticobject[i]->pos[2] + staticobject[i]->size[2] - moveColliderTerrain[1] ) / sizeColliderTerrain[1];
			...
			if( i == 0 && staticobject[i]->contact.points.size() > 0 )
			{
				int k__;
				printf( "\n\n\n    Number of contactpoints: " );
				for( k__ = 0; k__ < staticobject[i]->contact.points.size(); ++k__ )
				{
					printf( " :: %i", staticobject[i]->contact.points[k__].size() );
				}
//				printf( "\n    Number of contactpoints: %i", staticobject[i]->contact.points.size() );
			}
			
			staticobject[i]->ReactAfterCollisionDetection();
