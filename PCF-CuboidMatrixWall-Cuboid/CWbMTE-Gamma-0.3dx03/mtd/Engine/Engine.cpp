...
inline void Engine::Init( int * argc, char **argv )
{
	...
	DEBUG("after init sound");
	
	glutInit( argc, argv );
	
	DEBUG("after glutInit");
	...
	DEBUG("after graphic init");
	
	
	ScreenLoadTextures();
	
	DEBUG("after textures load");
	...
	DEBUG("befor setting up glut functions");
	glutReshapeFunc( Reshape__ );
	DEBUG("first glut function setted up");
	glutDisplayFunc( Renderer__ );
	DEBUG("second glut function setted up");
	glutTimerFunc( CWBMT_FRAMEINMS, FrameFunction__, 0 );
	DEBUG("third glut function setted up" );
	
	DEBUG("before load gl func");
...
void Engine::BeginLoop()
{
	...
	{
		(*(tab+i)) = i;
		_beginthread( PhysicObjectCollision__, 0, (void*)(tab+i) );
	}
	
	_beginthread( PhysicLoop__, 0, NULL );
	
	DEBUG("before glutMainLoop()");
...
Engine::Engine()
{
	...
	ActualAddedIdLightnings = 0;
	
	deleteVBOs = false;
	
	lightVertexCalculated = 0;
	NumberLightVertexCalculatedPerFrame = 15;
}
...
Engine::~Engine()
{
	...
	lightVertexCalculated = 0;
	NumberLightVertexCalculatedPerFrame = 0;
}
