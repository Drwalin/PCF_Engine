...
{
	DEBUG("befor CheckEngineThreadsInfoRendererThread()");
	CheckEngineThreadsInfoRendererThread();
	DEBUG("befor PhysicIteration()");
	PhysicIteration();
	
	GraphicTime = clock() - BeginGraphicTime;
	BeginGraphicTime = clock();
	DEBUG("befor CalculateDynamicLight()");
    ...
    glMatrixMode( GL_PROJECTION );
    glLoadIdentity();
    glViewport( 0, 0, 800, 600 );//(GLsizei)glutGet( GLUT_WINDOW_WIDTH ), (GLsizei)glutGet( GLUT_WINDOW_HEIGHT ) );
	...
	DEBUG("after gl translating");
	
	map.Draw( GraphicTime );
	DEBUG("after map.Draw()");
	GUIFunction( GraphicTime );
	DEBUG("after GUIFunction()");
	glFlush();
	DEBUG("after glFlush()");
	glutSwapBuffers();
	DEBUG("after glutSwapBuffers()" );
//	if( true )	// update dynamic light sources
	...
	DEBUG("after updating dynamic light" );
}
