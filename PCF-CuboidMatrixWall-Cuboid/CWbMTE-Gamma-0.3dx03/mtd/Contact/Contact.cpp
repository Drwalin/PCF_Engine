...
inline Vector Contact::GetBarycenter()
{
	Vector barycenter(0.0f,0.0f,0.0f);
	int i, j, count = 0;
	for( i = 0; i < points.size(); ++i )
		for( j = 0; j < points[i].size(); ++j, ++count )
			barycenter += points[i][j];
	if( count == 0 )
		return barycenter;
	return barycenter*(1.0f/(float(count)));
}
