...
void Map::Draw( float FrameTime )
{
	...
	glTranslatef( -(staticobject[0]->pos[0]+(staticobject[0]->size[0]*0.5f)), -(staticobject[0]->pos[1]+(staticobject[0]->size[1]*0.9f)), -(staticobject[0]->pos[2]+(staticobject[0]->size[2]*0.5f)) );
	...
	if( updateVBOs == true )
	{
		...
		{
			printf( "\n loading map model:   %s  ||  %i", graphicMapModels[i]->name, graphicMapModels[i] -> VBO.textureVBO );
			graphicMapModels[i] -> Init( graphicMapModels[i]->name, graphicMapModels[i] -> VBO.textureVBO );
			printf( "\n map model loaded:   %s  ||  %i", graphicMapModels[i]->name, graphicMapModels[i] -> VBO.textureVBO );
			printf( "\n Mapp: ___ %i  ||  %i", graphicMapModels[i] -> VBO.vertices.size(), graphicMapModels[i] -> VBO.indices.size() );
		}
		updateVBOs = false;
	...
	for( i = 0; i < staticobject.size(); i++ )
	{
		...
		if( staticobject[i]->model == NULL )
		{
			...
			glDisable( GL_TEXTURE_2D );
			glBegin( GL_LINES );
				glVertex3fv( &( ( staticobject[i]->GetVertex(0) + staticobject[i]->pos )[0] ) );
				glVertex3fv( &( ( staticobject[i]->GetVertex(1) + staticobject[i]->pos )[0] ) );
				
				glVertex3fv( &( ( staticobject[i]->GetVertex(1) + staticobject[i]->pos )[0] ) );
				glVertex3fv( &( ( staticobject[i]->GetVertex(2) + staticobject[i]->pos )[0] ) );
				
				glVertex3fv( &( ( staticobject[i]->GetVertex(2) + staticobject[i]->pos )[0] ) );
				glVertex3fv( &( ( staticobject[i]->GetVertex(3) + staticobject[i]->pos )[0] ) );
				
				glVertex3fv( &( ( staticobject[i]->GetVertex(3) + staticobject[i]->pos )[0] ) );
				glVertex3fv( &( ( staticobject[i]->GetVertex(0) + staticobject[i]->pos )[0] ) );
				
				glVertex3fv( &( ( staticobject[i]->GetVertex(4) + staticobject[i]->pos )[0] ) );
				glVertex3fv( &( ( staticobject[i]->GetVertex(5) + staticobject[i]->pos )[0] ) );
				
				glVertex3fv( &( ( staticobject[i]->GetVertex(5) + staticobject[i]->pos )[0] ) );
				glVertex3fv( &( ( staticobject[i]->GetVertex(6) + staticobject[i]->pos )[0] ) );
				
				glVertex3fv( &( ( staticobject[i]->GetVertex(6) + staticobject[i]->pos )[0] ) );
				glVertex3fv( &( ( staticobject[i]->GetVertex(7) + staticobject[i]->pos )[0] ) );
