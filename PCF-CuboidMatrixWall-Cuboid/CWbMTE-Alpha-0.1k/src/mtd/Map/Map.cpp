...
//	glEnd();
	...
	*/
	...
	engineInstantiation->SegmentCollision(
						...
						engineInstantiation->map.staticobject[1].pos+(engineInstantiation->map.staticobject[1].aabb[0].size*SetVector(0.5f,0.8f,0.5f)) + matrix.GetVertex( SetVector( 0.0f, 0.0f, 1000.0f ) ),
						&object, &triangle );
	...
	glBegin( GL_LINES );
		...
		glVertex3fv( &( ( engineInstantiation->map.staticobject[1].pos+(engineInstantiation->map.staticobject[1].aabb[0].size*SetVector(0.5f,0.8f,0.5f)) + matrix.GetVertex( SetVector( 0.0f, 0.0f, 1000.0f ) ) )[0] ) );
	glEnd();
