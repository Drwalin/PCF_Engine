...
inline void ShadowMap::Generate( Map * map )
{
	...
	for( int i = 0; i < map->triPHYS.size(); ++i )
	{
		...
		if( i*100/(map->triPHYS.size()) != (i+1)*100/(map->triPHYS.size()) )
			printf( " ." );
	}
	...
	printf( " Done" );
	printf( "\n Number of Shadow map triangles: %i\n Number of Shadow map vertices: %i", VBO.indices.size() / 3, VBO.vertices.size() );
}
