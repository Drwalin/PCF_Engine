...
int main( int argc, char *argv[] )
{
    ...
    game.engine.map.texture[0].LoadInit( "Media\\texture1.bmpx", GL_LINEAR );
    game.engine.map.vbotex.resize( 1 );
    game.engine.map.vbotex[0].SetTexture( "Media\\textmodel.bmpx", GL_LINEAR );
	/*
    	load textures and models:
    */
    game.engine.AddGraphicModel( "Herobrine.obj", "Media\\Herobrine.bmpx", GL_NEAREST );
    //game.engine.AddGraphicModel( "Tree2.obj", "liscie.bmpx", GL_NEAREST );
	...
	game.engine.map.staticobject.resize( 200 );
	for( int i = 0; i < game.engine.map.staticobject.size(); i++ )
	{
		...
		if( i != 0 )
			game.engine.map.staticobject[i].model = &(game.engine.GraphicModels[0]);
	}
