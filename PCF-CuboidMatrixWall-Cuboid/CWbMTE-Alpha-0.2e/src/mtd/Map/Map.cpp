	...
	glDepthFunc( GL_LESS );
		...
		for( i = 0; i < staticobject.size(); i++ )
		{
			...
			else
			{
				...
				glScalef( ((staticobject[i].max-staticobject[i].min)[0]*3.0f*0.5f), ((staticobject[i].max-staticobject[i].min)[1]), ((staticobject[i].max-staticobject[i].min)[2]) );
				staticobject[i].model -> Draw();
				glScalef( 1.0f/((staticobject[i].max-staticobject[i].min)[0]*3.0f*0.5f), 1.0f/((staticobject[i].max-staticobject[i].min)[1]), 1.0f/((staticobject[i].max-staticobject[i].min)[2]) );
				glRotatef( -90.0f-staticobject[i].see[0], 0.0f, 1.0f, 0.0f );
				glTranslatef( -(staticobject[i].min[0]+staticobject[i].max[0])*0.5f, -staticobject[i].pos[1], -(staticobject[i].min[2]+staticobject[i].max[2])*0.5f );
			}
		}
