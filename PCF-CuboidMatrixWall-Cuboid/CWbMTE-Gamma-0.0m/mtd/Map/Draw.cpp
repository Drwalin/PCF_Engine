...
void Map::Draw( float FrameTime )
{
	...
	if( updateVBOs == true )
	{
		...
		{
			printf( "\n loading map model:   %s  ||  %i", graphicMapModels[i]->name, 0 );//graphicMapModels[i] -> VBO.textureVBO );
			graphicMapModels[i] -> Init( graphicMapModels[i]->name, 0 );//graphicMapModels[i] -> VBO.textureVBO );
			printf( "\n map model loaded:   %s  ||  %i", graphicMapModels[i]->name, 0 );//graphicMapModels[i] -> VBO.textureVBO );
