...
int main( int argc, char *argv[] )
{
	...
	float x, z;
	float minx = -250.0f, minz = -250.0f, maxx = 50.0f, maxz = 50.0f;
	...
	for( x = minx; x < maxx; x += size )
	{
		for( z = minz; z < maxz; z += size )
		{
			...
			game.engine.AddVertexToLastVBO( x,      getheight(x,z), z,      0.0f, 1.0f );
			game.engine.AddVertexToLastVBO( x+size, getheight(x+size,z), z,      1.0f, 1.0f );
			game.engine.AddVertexToLastVBO( x+size, getheight(x+size,z+size), z+size, 1.0f, 0.0f );
			game.engine.CreateTriangleWithLastThreeVertices();
			
			game.engine.AddVertexToLastVBO( x,      getheight(x,z), z,      0.0f, 1.0f );
			game.engine.AddVertexToLastVBO( x,      getheight(x,z+size), z+size, 0.0f, 0.0f );
			game.engine.AddVertexToLastVBO( x+size, getheight(x+size,z+size), z+size, 1.0f, 0.0f );
			game.engine.CreateTriangleWithLastThreeVertices();
