...
void GUIFunction( float FrameTime )
{
	...
	for( i = 0; i < engineInstantiation->map.lightnings.size(); i++ )
	{
...
//			engineInstantiation->map.lightnings.erase( engineInstantiation->map.lightnings.begin() + i );
//			i--;
		}
	}//engineInstantiation->map.lightnings.Update( FrameTime );
...
//	engineInstantiation->map.particles.AddParticle( SetVector(0.0f,-5.5f,0.0f), 0.5f, SetVector(0.0f,0.0f,0.0f), SetVector(0.0f,0.01f,0.0f), 1.8f, 0, 0, 0, 30, 30, 30, 30.0f, 5.0f, 1 );
	...
	*(tabcolor_____col+0) = 0.05f;
	*(tabcolor_____col+0) = 0.05f;
	*(tabcolor_____col+0) = 1.0f;
	drawString( "Dziala?", 100, 100, tabcolor_____col );
...
//	sprintf( FPS[1], "Physic FPS:  %f", timefps[2] );
    ...
    glEnable( GL_TEXTURE_2D );
		...
		//if( AddLightings > 0 )
		{
			...
			ActualAddedIdLightnings %= engineInstantiation->map.lightnings.size();
			printf( "\n %i", ActualAddedIdLightnings );
			engineInstantiation->map.lightnings[ ActualAddedIdLightnings ].Set(
