			...
			for( i = 0; i < colliderter[tris[0]][tris[1]].tri.size(); i++ )
			{
				if( colliderter[tris[0]][tris[1]].tri[i] != *triangle )
				{
					...
					if( dot != 0.0f )
					{
						...
						if( dot >= 0.0f && dot <= lenght && int(float((((p1+(direction*t))[0]) - (moveColliderTerrain[0])) / (sizeColliderTerrain[0]))) == tris[0] && int(float((((p1+(direction*t))[0]) - (moveColliderTerrain[0])) / (sizeColliderTerrain[0]))) == tris[1] )
						{
							if( colliderter[tris[0]][tris[1]].tri[i] -> PointIn( p1 + (direction * dot) ) == true )
							{
								if( *triangle == NULL )
								{
			...
			for( i = 0; i < colliderobj[objs[0]][objs[1]].staticmodel.size(); i++ )
			{
				if( colliderobj[objs[0]][objs[1]].staticmodel[i] != *object && decludingobject != colliderobj[objs[0]][objs[1]].staticmodel[i] )
				{
					if( colliderobj[objs[0]][objs[1]].staticmodel[i] -> RayCollisionUse == true )
					{
						...
						{
							collision.RayAABB( p1, p2, colliderobj[objs[0]][objs[1]].staticmodel[i], &FastPointContactAABB );
							if( *object == NULL )
							{
