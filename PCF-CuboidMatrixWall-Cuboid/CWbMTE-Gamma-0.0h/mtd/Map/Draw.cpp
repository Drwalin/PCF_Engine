...
void Map::Draw( float FrameTime )
{
	...
	for( i = 0; i < staticobject.size(); i++ )
	{
		...
		if( staticobject[i]->model == NULL || i == 0 || DebugMode == true )
		{
			glLineWidth(2.0);
			glDepthFunc( GL_LESS );
			glDisable( GL_BLEND );
		...
		{
			glTranslatef( staticobject[i]->drawpos[0]+(staticobject[i]->size[0]*0.5f), staticobject[i]->drawpos[1]+(staticobject[i]->size[1]*0.5f), staticobject[i]->drawpos[2]+(staticobject[i]->size[2]*0.5f) );
