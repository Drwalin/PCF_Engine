...
void Map::Draw( float FrameTime )
{
	...
	if( updateVBOs == true )
	{
		...
		{
			glDeleteBuffers( 1, &(graphicMapModels[i] -> VBO.indicesVBOid) );
			glDeleteBuffers( 1, &(graphicMapModels[i] -> VBO.verticesVBOid) );
			graphicMapModels[i] -> VBO.indicesVBOid = 0;
			graphicMapModels[i] -> VBO.verticesVBOid = 0;
			graphicMapModels[i] -> VBO.Generate();
		}
		updateVBOs = false;
	}
	for( i = 0; i < vbotex.size(); i++ )
