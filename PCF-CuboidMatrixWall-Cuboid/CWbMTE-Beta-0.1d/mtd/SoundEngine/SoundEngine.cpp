...
inline void SoundEngine::Update()
{
	...
	Vector pos;
	pos.Set( (engineInstantiation->Object(0)->pos[0]+(engineInstantiation->Object(0)->aabb[0].size[0]*0.5f)), (engineInstantiation->Object(0)->pos[1]+(engineInstantiation->Object(0)->aabb[0].size[1]*0.9f)), (engineInstantiation->Object(0)->pos[2]+(engineInstantiation->Object(0)->aabb[0].size[2]*0.5f)) );
	...
	for( i = 0; i < sounds.size(); i++ )
	{
		...
		else
		{
			...
			{
				sndVolume = pow( (double)(1.0f-sndVolume), 3.0 );
				sndVolume = log( (/*HearDistance+*/sounds[i].range*sounds[i].range) / (sndVolume*sndVolume) );
			}
		}
