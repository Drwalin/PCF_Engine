...
inline unsigned int Collision::AABBTriangle( AABB * object, TrianglePX * tri, float time )
{
	int i;
	...
	//if( false )
	{
		...
		if( ForceDistance < 0.0f )
		{
			...
			//object -> resistanceforce = ((object -> resistanceforce)) - ( (0.0f - object -> vel).Versor() * abs(ForceDistance) );
		}
