...
int main( int argc, char *argv[] )
{
    ...
    */
	...
	engine.map.staticobject.resize( 100 );
	for( int i = 0; i < engine.map.staticobject.size(); i++ )
	{
		engine.map.staticobject[i].type = PXMODEL_AABB;
		engine.map.staticobject[i].object = new AABB;
		((AABB*)(engine.map.staticobject[i].object))->size.Set( 0.6f, 1.75f, 0.6f );
		((AABB*)(engine.map.staticobject[i].object))->pos.Set( 0.0f, 30.0f, 0.0f );
