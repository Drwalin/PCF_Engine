...
inline int  Map::SegmentCollision( Vector p1, Vector p2, TrianglePX ** triangle, Vector * point )
{
	*triangle = NULL;
	...
	Vector FastPointContactTriangle, flatdirection;
	flatdirection.Set( direction[0], 0.0f, direction[2] );
	...
	for( t = 0.0f; t < lenght; t += EPSILON )
	{
		...
		if( *triangle != NULL )
		{
			...
			if( tris[0] == int(float(((ff[0]) - (moveColliderTerrain[0])) / (sizeColliderTerrain[0]))) )//////////////////////////////////////////////////////////////////////////
			{
				if( tris[1] == int(float(((ff[2]) - (moveColliderTerrain[1])) / (sizeColliderTerrain[1]))) )//////////////////////////////////////////////////////////////////////
				{
					(*triangle) -> GetCrossPoint( p1, p2, point );
					return COLLISION_WITH_TRIANGLE;
				}
			}
		}
...
inline int  Map::SegmentCollision( Vector p1, Vector p2, Vector * point )
{
	TrianglePX * triangle = NULL;
