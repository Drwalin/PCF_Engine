...
float * tabcolor_____col = new float[4];
...
void GUIFunction( float FrameTime )
{
	...
	for( i = 0; i < engineInstantiation->map.NumberOfLightnings; i++ )
	{
		engineInstantiation->map.lightnings[i].Update( FrameTime );
		if( engineInstantiation->map.lightnings[i].actualTime > engineInstantiation->map.lightnings[i].EndingTime )
		{
			if( engineInstantiation->map.lightnings[i].EndingTime != 0.0f )
			{
				engineInstantiation->map.lightnings[i].EndingTime = 0.0f;
				delete[] engineInstantiation->map.lightnings[i].vertex.childs;
				engineInstantiation->map.lightnings[i].vertex.childs = NULL;
			}
		}
	}
...
//	engineInstantiation->map.particles.AddParticle( SetVector(0.0f,-5.5f,0.0f), 0.5f, SetVector(0.0f,0.0f,0.0f), SetVector(0.0f,0.01f,0.0f), 1.8f, 0, 0, 0, 30, 30, 30, 30.0f, 5.0f, 1 );
	...
	*(tabcolor_____col+0) = 1.0f;
	*(tabcolor_____col+1) = 0.05f;
	*(tabcolor_____col+2) = 0.05f;
	*(tabcolor_____col+3) = 1.0f;
	drawString( "Dziala?", 10, 10, tabcolor_____col );
...
//	sprintf( FPS[1], "Physic FPS:  %f", timefps[2] );
    ...
    glEnable( GL_TEXTURE_2D );
		...
		{
            engineInstantiation->map.staticobject[0]->see[1] += 360.0f * FrameTime / 2.0f;
		}
		...
		//if( AddLightings > 0 )
		{
			...
			ActualAddedIdLightnings %= engineInstantiation->map.NumberOfLightnings;
			engineInstantiation->map.lightnings[ ActualAddedIdLightnings % engineInstantiation->map.NumberOfLightnings ].Set(
						SetVector( (engineInstantiation->map.staticobject[0]->pos[0]+(engineInstantiation->map.staticobject[0]->aabb[0].size[0]*0.5f)), (engineInstantiation->map.staticobject[0]->pos[1]+(engineInstantiation->map.staticobject[0]->aabb[0].size[1]*0.9f)), (engineInstantiation->map.staticobject[0]->pos[2]+(engineInstantiation->map.staticobject[0]->aabb[0].size[2]*0.5f)) ),
						SetVector( (engineInstantiation->map.staticobject[0]->pos[0]+(engineInstantiation->map.staticobject[0]->aabb[0].size[0]*0.5f)), (engineInstantiation->map.staticobject[0]->pos[1]+(engineInstantiation->map.staticobject[0]->aabb[0].size[1]*0.9f)), (engineInstantiation->map.staticobject[0]->pos[2]+(engineInstantiation->map.staticobject[0]->aabb[0].size[2]*0.5f)) ) + ( matrix.GetVertex(forward) * ((float((rand()%10000)-5000)/100.0f)+70.0f) ),
						25.0f, 1.0f,
						0.4f, 0.4f, 1.0f,
						0.8f, 0.8f, 1.0f,
						10,
