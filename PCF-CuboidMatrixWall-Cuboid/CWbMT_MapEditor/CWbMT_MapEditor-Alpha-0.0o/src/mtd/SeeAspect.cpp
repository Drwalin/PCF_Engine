...
inline void SeeAspect::Draw()
{
	...
	if( win != NULL )
	{
		...
		for( i = 0; i < vertex.size(); i++ )
		{
			...
			ALLEGRO_VERTEX * vert_draw = new ALLEGRO_VERTEX[3];
			
			Vector fast_vec;
			
			for( i = 0; i+2 < indices.size(); i++ )
			{
				/*
				fast_vec = win->matrix.GetVertex( vertex[indices[i]].pos );
				
				(vert_draw+0)->x = fast_vec[0];
				(vert_draw+0)->y = fast_vec[1];
				(vert_draw+0)->z = fast_vec[2];
				(vert_draw+0)->u = int(vertex[indices[i]][3]*float(al_get_bitmap_width(FullMapTexture)));
				(vert_draw+0)->v = al_get_bitmap_height(FullMapTexture)-int(vertex[indices[i]][4]*float(al_get_bitmap_height(FullMapTexture)));
				
				al_draw_prim(vert_draw, NULL, FullMapTexture, 0, 3, ALLEGRO_PRIM_TRIANGLE_LIST);
				
				i++;////
				
				fast_vec = win->matrix.GetVertex( vertex[indices[i]].pos );
				
				(vert_draw+1)->x = fast_vec[0];
				(vert_draw+1)->y = fast_vec[1];
				(vert_draw+1)->z = fast_vec[2];
				(vert_draw+1)->u = int(vertex[indices[i]][3]*float(al_get_bitmap_width(FullMapTexture)));
				(vert_draw+1)->v = al_get_bitmap_height(FullMapTexture)-int(vertex[indices[i]][4]*float(al_get_bitmap_height(FullMapTexture)));
				...
				(vert_draw+2)->x = fast_vec[0];
				(vert_draw+2)->y = fast_vec[1];
				(vert_draw+2)->z = fast_vec[2];
				(vert_draw+2)->u = int(vertex[indices[i]][3]*float(al_get_bitmap_width(FullMapTexture)));
				(vert_draw+2)->v = al_get_bitmap_height(FullMapTexture)-int(vertex[indices[i]][4]*float(al_get_bitmap_height(FullMapTexture)));
