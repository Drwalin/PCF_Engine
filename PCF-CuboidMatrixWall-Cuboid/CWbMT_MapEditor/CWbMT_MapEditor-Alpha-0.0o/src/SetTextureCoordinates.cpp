...
inline void SetTextureCoordinaes()
{
	...
	while( !isKeyDown(ALLEGRO_KEY_ENTER) && !isRMBDown() )
	{
	...
	vertex[ActivatedVertex][3] = float((mouse.x/scale)+(mx))/float(texwidth);
	vertex[ActivatedVertex][4] = float(texheight-((mouse.y/scale)+(my)))/float(texheight);
}
