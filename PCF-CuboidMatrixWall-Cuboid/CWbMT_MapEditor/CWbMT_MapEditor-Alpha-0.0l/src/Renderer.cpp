...
void Renderer()
{
	...
	if( ActivatedWindow >= 0 && ActivatedWindow < window.size() )
	{
	    ...
	    DrawString( 2, 8, 0, 434, possition[2] );
	}
    sprintf( possition[0], "Vertex: %i", vertex.size() );
    sprintf( possition[4], "Triangles: %i", triangles.size() );
    ...
    DrawString( 2, 8, 0, 450, possition[0] );
    DrawString( 2, 8, 0, 466, possition[1] );
    DrawString( 2, 8, 0, 484, possition[4] );
