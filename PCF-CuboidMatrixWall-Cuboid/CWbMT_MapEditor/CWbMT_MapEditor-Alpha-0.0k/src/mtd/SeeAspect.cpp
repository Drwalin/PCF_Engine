...
//			GetPoint( win->points[i][0], win->points[i][1], &_x, &_y );
		...
		{
			Vector fast_vec1, fast_vec2;
			for( i = 0; i < triangles.size(); i++ )
			{
				fast_vec1 = triangles[i].GetBeginNormal();
				fast_vec2 = fast_vec1 + triangles[i].normal;
				GetPoint( fast_vec1[0], fast_vec1[1], &bx, &by );
				GetPoint( fast_vec2[0], fast_vec2[1], &ex, &ey );
