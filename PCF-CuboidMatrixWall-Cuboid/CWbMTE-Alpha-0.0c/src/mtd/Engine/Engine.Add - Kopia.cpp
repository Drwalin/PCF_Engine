...
inline void Engine::Add( int arg, ... )
{
    ...
    case CMT_OBJECT:
        ...
        map.object[ map.object.size()-1 ].collision_calculate = va_arg( arglist, bool );
        map.object[ map.object.size()-1 ].gravity_work = va_arg( arglist, bool );
    ...
    // Walls:
    case CMT_WALL:
        map.wall.resize( map.wall.size()+1 );
        map.wall[ map.wall.size()-1 ].pos[0] = va_arg( arglist, float );
        map.wall[ map.wall.size()-1 ].pos[1] = va_arg( arglist, float );
        map.wall[ map.wall.size()-1 ].pos[2] = va_arg( arglist, float );
        map.wall[ map.wall.size()-1 ].size[0] = va_arg( arglist, float );
        map.wall[ map.wall.size()-1 ].size[1] = va_arg( arglist, float );
        map.wall[ map.wall.size()-1 ].size[2] = va_arg( arglist, float );
        map.wall[ map.wall.size()-1 ].rotate[0] = va_arg( arglist, float );
        map.wall[ map.wall.size()-1 ].rotate[1] = va_arg( arglist, float );
        map.wall[ map.wall.size()-1 ].rotate[2] = va_arg( arglist, float );
        map.wall[ map.wall.size()-1 ].texture = va_arg( arglist, int );
        int * id_wall = va_arg( arglist, *int );
        if( id_wall != NULL )
            *id_wall = map.wall.size()-1;
        va_end( arglist );
        return;
        
    case CMT_WALL_POS:
        int id_wall = va_arg( arglist, int );
        map.wall[ id_wall ].pos[0] += va_arg( arglist, float );
        map.wall[ id_wall ].pos[1] += va_arg( arglist, float );
        map.wall[ id_wall ].pos[2] += va_arg( arglist, float );
        va_end( arglist );
        return;
        
    case CMT_WALL_SIZE:
        int id_wall = va_arg( arglist, int );
        map.wall[ id_wall ].size[0] += va_arg( arglist, float );
        map.wall[ id_wall ].size[1] += va_arg( arglist, float );
        map.wall[ id_wall ].size[2] += va_arg( arglist, float );
        va_end( arglist );
        return;
        
    case CMT_WALL_ROTATE:
        int id_wall = va_arg( arglist, int );
        map.wall[ id_wall ].rotate[0] += va_arg( arglist, float );
        map.wall[ id_wall ].rotate[1] += va_arg( arglist, float );
        map.wall[ id_wall ].rotate[2] += va_arg( arglist, float );
        va_end( arglist );
        return;
    ...
    case CMT_WALL_V:
        ...
        map.wall.resize( map.wall.size()+1 );
        map.wall[ map.wall.size()-1 ].pos[0] = pos_wall[0];
        map.wall[ map.wall.size()-1 ].pos[1] = pos_wall[1];
        map.wall[ map.wall.size()-1 ].pos[2] = pos_wall[2];
        map.wall[ map.wall.size()-1 ].size[0] = size_wall[0];
        map.wall[ map.wall.size()-1 ].size[1] = size_wall[1];
        map.wall[ map.wall.size()-1 ].size[2] = size_wall[2];
        map.wall[ map.wall.size()-1 ].rotate[0] = rotate_wall[0];
        map.wall[ map.wall.size()-1 ].rotate[1] = rotate_wall[1];
        map.wall[ map.wall.size()-1 ].rotate[2] = rotate_wall[2];
        map.wall[ map.wall.size()-1 ].texture = va_arg( arglist, int );
        int * id_wall = va_arg( arglist, *int );
        if( id_wall != NULL )
            *id_wall = map.wall.size()-1;
    ...
    case CMT_OBJECT_POS_V:
        ...
        map.wall[ id_wall ].pos[0] += pos_wall[0];
        map.wall[ id_wall ].pos[1] += pos_wall[1];
        map.wall[ id_wall ].pos[2] += pos_wall[2];
        va_end( arglist );
        return;
    ...
    case CMT_OBJECT_SIZE_V:
        ...
        map.wall[ id_wall ].size[0] += size_wall[0];
        map.wall[ id_wall ].size[1] += size_wall[1];
        map.wall[ id_wall ].size[2] += size_wall[2];
        va_end( arglist );
        return;
    ...
    case CMT_OBJECT_ROTATE_V:
        ...
        map.wall[ id_wall ].rotate[0] += rotate_wall[0];
        map.wall[ id_wall ].rotate[1] += rotate_wall[1];
        map.wall[ id_wall ].rotate[2] += rotate_wall[2];
        va_end( arglist );
        return;
    ...
    // Textures:
    case CMT_TEXTURE:
        map.texture.resize( map.texture.size()+1 );
        sprintf( map.texture[ map.texture.size()-1 ].name, "%s", va_arg( arglist, *char ) );
