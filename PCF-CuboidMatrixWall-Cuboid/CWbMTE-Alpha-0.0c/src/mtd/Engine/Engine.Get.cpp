...
inline void Engine::Get( int arg, ... )
{
    va_list arglist;
    ...
    // Graphic mode:
    case CMT_GRAPHIC_MODE_OPENGL:
        int * graphicemode;
        graphicemode = va_arg( arglist, (int*) );
        *graphicemode = GraphicMode;
        va_end( arglist );
        return; // break;
        break;
    ...
    case CMT_GRAVITY:
        float * gravity;
        gravity = va_arg( arglist, (float*) );
        *gravity = map[ActuallyMapUsing].gravity;
        va_end( arglist );
        return; // break;
        break;
    
    case CMT_AIR_RESISTANCE:
        float * airresistance;
        airresistance = va_arg( arglist, (float*) );
        *airresistance = map[ActuallyMapUsing].AirResistance;
        va_end( arglist );
        return; // break;
        break;
    
    // Camera:
    case CMT_CAMER_ID_OBJECT:
        int * cameraid;
        cameraid = va_arg( arglist, (int*) );
        *cameraid = CameraObject;
        va_end( arglist );
        return; // break;
        break;
