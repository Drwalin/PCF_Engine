...
//	matrix.RotateX( engineInstantiation->map.staticobject[0].see[1] );
	...
	*/
	...
	if( keyboardobiect.keypressed[GameInstantiation->Jump] && engineInstantiation->map.staticobject[0].aabb[0].CollisionDown )//keyboardobiect.downkeypressed[GameInstantiation->Jump] )	// SPACE
	{
		...
		engineInstantiation->map.staticobject[0].vel[1] += 1.6f;//FORCE_DEFINE;
	}
