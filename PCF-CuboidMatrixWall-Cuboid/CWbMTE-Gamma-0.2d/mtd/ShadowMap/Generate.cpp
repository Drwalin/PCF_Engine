...
inline void ShadowMap::Generate( Map * map )
{
	...
	int i, j, k, l, num, a, b, c, d, e, a_, b_, c_, d_, e_;
//	float d1, d2;
	Vector v1, v2, v3, v4;
	float f1, f2, f3, f4;
			...
			else
			{
				...
				for( j = 0; j < num; ++j )
				{
					for( k = 0; k < (num-j); ++k )
					{
						...
						/*a_ = GetIndiceId( beg + (dir1*float(j))        + (dir2*float(k)) );
						b_ = GetIndiceId( beg + (dir1*float(j+1))      + (dir2*float(k)) );
						c_ = GetIndiceId( beg + (dir1*(float(j)+0.5f)) + (dir2*(float(k)+0.5f)) );
						d_ = GetIndiceId( beg + (dir1*float(j))        + (dir2*float(k+1)) );
						
						a = a_; b = b_; c = c_;
						VBO.AddTriangle( a, b, c );
						a = a_; d = d_; c = c_;
						VBO.AddTriangle( a, c, d );*/
						...
						if( (k+1) < (num-j) )
						{
							...
							/*e_ = GetIndiceId( beg + (dir1*float(j+1))      + (dir2*float(k+1)) );
							
							e = e_; d = d_; c = c_;
							VBO.AddTriangle( c, d, e );
							e = e_; b = b_; c = c_;
							VBO.AddTriangle( b, c, e );*/
						}
					}
				}
			}
