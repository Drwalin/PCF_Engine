...
inline type& Octree::operator[] ( int id )
{
	return items[id];
}

inline void Octree::AddItem( type src )
{
	items.push_back( src );
}
...
inline void OperateOctree::SetAt( Vector begin, Vector end, type src )
{
	begin = begin - offset;
	end = end - offset;
...
inline vector < type > OperateOctree::GetFrom( Vector begin, Vector end )
{
	int i = 0, j = 0;
	vector < type > tab;
