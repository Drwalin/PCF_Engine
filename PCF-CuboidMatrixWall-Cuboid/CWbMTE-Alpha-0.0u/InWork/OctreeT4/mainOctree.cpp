...
void PrintOctree( Octree<int> * octree )
{
	if( octree -> next != NULL )
	{
...
//		octree.octree = new Octree;
	...
	octree.maxChildren = 10;
	
	Data fast;
	
	
	printf( "Begin load time: %i", clock() );
	...
	for( float x = -80.0f; x < 98.0f; x ++ )
	{
		for( float y = -10.0f; y < 70.0f; y ++ )
		{
			for( float z = -90.0f; z < 80.0f; z ++ )
			{
				...
				fast.a = obj.size()+1;
				obj.push_back( fast );
				octree.SetAt( obj[ obj.size()-1 ].pos, obj[ obj.size()-1 ].pos+obj[ obj.size()-1 ].size, obj.size() );
	...
	*/
	...
	while( 1 )
	{
		...
		case 'L':
			//fast.Load();
			//obj.push_back( fast );
			//octree.SetAt( obj[ obj.size()-1 ].pos, obj[ obj.size()-1 ].pos+obj[ obj.size()-1 ].size, &obj[ obj.size()-1 ] );
			break;
