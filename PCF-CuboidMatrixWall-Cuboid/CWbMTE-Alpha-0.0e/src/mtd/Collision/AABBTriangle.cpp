...
inline unsigned int Collision::AABBTriangle( AABB * object, Vector offset1, TrianglePX * tri, float time )
{
	...
	{	// vertex 1 : [0;0;0]
		...
		if( vertex[0] < end && vertex[0] > begin ) collision = true;
	}
	{	// vertex 1 : [0;0;1]
		vertex[1] = tri -> ClosestPoint( begin + SetVector( 0.0f, 0.0f, size_[2] ) );
		if( vertex[1] < end && vertex[1] > begin ) collision = true;
	}
	{	// vertex 1 : [0;1;0]
		vertex[2] = tri -> ClosestPoint( begin + SetVector( 0.0f, size_[1], 0.0f ) );
		if( vertex[2] < end && vertex[2] > begin ) collision = true;
	}
	{	// vertex 1 : [0;1;1]
		vertex[3] = tri -> ClosestPoint( begin + SetVector( 0.0f, size_[1], size_[2] ) );
		if( vertex[3] < end && vertex[3] > begin ) collision = true;
	}
	{	// vertex 1 : [1;0;0]
		vertex[4] = tri -> ClosestPoint( begin + SetVector( size_[0], 0.0f, 0.0f ) );
		if( vertex[4] < end && vertex[4] > begin ) collision = true;
	}
	{	// vertex 1 : [1;0;1]
		vertex[5] = tri -> ClosestPoint( begin + SetVector( size_[0], 0.0f, size_[2] ) );
		if( vertex[5] < end && vertex[5] > begin ) collision = true;
	}
	{	// vertex 1 : [1;1;0]
		vertex[6] = tri -> ClosestPoint( begin + SetVector( size_[0], size_[1], 0.0f ) );
		if( vertex[6] < end && vertex[6] > begin ) collision = true;
	}
	{	// vertex 1 : [1;1;1]
		vertex[7] = tri -> ClosestPoint( begin + size_ );
		if( vertex[7] < end && vertex[7] > begin ) collision = true;
	}
	{	// vertex 1 : [0.5;0.5;0.5]
		vertex[8] = tri -> ClosestPoint( begin + (size_ * 0.5f) );
		if( vertex[8] < end && vertex[8] > begin ) collision = true;
	}
	
	vertex[0] = object -> MaxWithVector( 0.0f - tri -> normal );
	//vertex[1] = object -> MaxWithVector( tri -> normal );
	
	//point[0] = tri -> ClosestPoint( vertex[0] );
	//point[1] = tri -> ClosestPoint( vertex[1] );
