...
inline unsigned int Collision::AABBTriangle( AABB * object, Vector * offset1, Vector backOffset1, TrianglePX * tri, float time )
{
	...
	if( object->MaxWithVector(normal
	...
	{
		{
			p1dot = ((object->MaxWithVector(tri->normal))-(tri->vertex[0])+(*offset1)).Dot(tri->normal);
			p2dot = ((object->MaxWithVector(0.0f-tri->normal))-(tri->vertex[0])+(*offset1)).Dot(tri->normal);
		...
		fastDot = ( contactBarycentric - ( tri -> vertex[0] ) ).Dot( tri -> normal );
		if( fastDot != 0.0f )
		{
			contactBarycentric -= ( tri -> normal ) * fastDot;
		}
		...
		{	// Calc max defiation from outside to contact barycentric
			MinWithNormal = contactBarycentric - ( (tri->normal) * 100.0f );
...
//				*offset1 += contactBarycentric - pointfast;
		...
		if( VelocityDistance <= 0.0f )
		{
			*(object -> vel) -= tri -> normal * ( VelocityDistance * 1.01f );
		}
