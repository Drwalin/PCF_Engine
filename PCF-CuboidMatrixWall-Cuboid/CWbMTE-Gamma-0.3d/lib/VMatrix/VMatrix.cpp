...
inline float & VMatrix::operator [] ( const int src )
{
	return m[src>>2][src%4];
}
...
inline Vector VMatrix::operator *  ( const Vector &src )const  // zwraca pomno¿on¹ aktualn¹ macierz przez podany wektor
{
    Vector out;
...
inline Vector VMatrix::GetVertexBack( Vector src )
{
    // punkt * macierz odwrotna = punkt przed przekrzta³ceniem
    return ((*this).GetReverse()).GetVertex( src );
}

inline VMatrix gluLookAtVMatrix( Vector eyePosition3D, Vector forward, Vector upVector3D )
{
	// code from: https://www.khronos.org/opengl/wiki/GluLookAt_code
	
	Vector side, up;
	VMatrix dstMat;
	
	side = ( forward && upVector3D ).Versor();
	up = side && forward;
	...
	dstMat.Translate( 0.0f - eyePosition3D );
