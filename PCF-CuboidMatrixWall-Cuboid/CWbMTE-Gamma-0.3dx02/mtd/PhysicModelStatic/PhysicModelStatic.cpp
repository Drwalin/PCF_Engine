...
inline Vector PhysicModelStatic::GetBarycentricDraw()
{
	return drawpos + (size*0.5f);
}
...
{
	vel += force * time / mass;
	if( abs(vel[0]) < 0.001f )	vel[0] = 0.0f;
	if( abs(vel[1]) < 0.001f )	vel[1] = 0.0f;
	if( abs(vel[2]) < 0.001f )	vel[2] = 0.0f;
	if( vel[0] == -0.0f ) vel[0] = 0.0f;
	if( vel[0] == -0.0f ) vel[0] = 0.0f;
	if( vel[0] == -0.0f ) vel[0] = 0.0f;
}
...
//			if( velocityDot < 0.0f )
//				vel -= contact.direction[i] * ( velocityDot * 1.0f );
...
//				force -= contact.direction[i] * ( forceDot * 1.0f );
		...
		velocityDot = direction.Versor().Dot( vel );
		forceDot = direction.Versor().Dot( force );
		if( velocityDot < 0.0f )
			vel -= direction.Versor() * ( velocityDot );
		if( forceDot < 0.0f )
			force -= direction.Versor() * ( forceDot );
	}
}
