...
void GUIFunction( float FrameTime )
{
	...
	sprintf( look[0], "Y look: %f", engine.map.staticobject[0].see[0] );
	sprintf( look[1], "Z look: %f", engine.map.staticobject[0].see[1] );
	sprintf( velocitylenght3D, "vel 3D: %f", engine.map.staticobject[0].vel.Lenght() );
	sprintf( velocitylenght2D, "vel 2D: %f", SetVector( engine.map.staticobject[0].vel[0], 0.0f, engine.map.staticobject[0].vel[2] ).Lenght() );
