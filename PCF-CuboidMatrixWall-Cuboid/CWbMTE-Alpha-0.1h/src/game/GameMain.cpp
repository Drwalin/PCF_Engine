...
int main( int argc, char *argv[] )
{
    ...
    game.engine.map.vbo.resize( 1 );
    game.engine.map.vbo[0].SetTexture( "Media\\texture1.bmpx" );
	/*
    ...
    */
	...
	for( float z = -SIZEEEE_2; z < SIZEEEE_2+ONETRIANGLESIZE; z+=ONETRIANGLESIZE )
	{
		...
		{
			#define VBO game.engine.map.vbo[0].AddVertex
//				( float x, float y, float z, float texcoordx, float texcoordy )
	...
	for( int z = 0; z < int(SIZEEEE*2.0f/ONETRIANGLESIZE); z++ )
	{
		...
		{
			#define VBO game.engine.map.vbo[0].AddTriangle
	...
	game.engine.map.vbo[0].Generate();
