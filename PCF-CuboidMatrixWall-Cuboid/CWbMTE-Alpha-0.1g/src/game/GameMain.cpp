...
int main( int argc, char *argv[] )
{
    ...
    game.engine.map.vbo.SetTexture( "Media\\texture1.bmpx" );
	/*
    ...
    */
	...
	game.engine.map.staticobject.resize( 500 );
	for( int i = 0; i < game.engine.map.staticobject.size(); i++ )
	{
		...
		game.engine.map.staticobject[i].pos.Set( 0.0f, 30.0f+float(i)*0.4f, 0.0f );
	...
	for( float z = -SIZEEEE_2; z < SIZEEEE_2+ONETRIANGLESIZE; z+=ONETRIANGLESIZE )
	{
		...
		{
			#define VBO game.engine.map.vbo.AddVertex
//				( float x, float y, float z, float texcoordx, float texcoordy )
	...
	for( int z = 0; z < int(SIZEEEE*2.0f/ONETRIANGLESIZE); z++ )
	{
		...
		{
			#define VBO game.engine.map.vbo.AddTriangle
   	...
   	for( float z = -SIZEEEE_2; z < SIZEEEE_2; z+=ONETRIANGLESIZE )
	{
		...
		//	goto PHYSX_TRIANGLES;
			...
			#define VBO game.engine.map.vbo.AddTriangle
				...
				VBO(
					(unsigned int)( ((x+SIZEEEE_2)/ONETRIANGLESIZE) + (2.0f*SIZEEEE_2*(z+SIZEEEE_2)/ONETRIANGLESIZE) ),
					(unsigned int)( ((x+SIZEEEE_2)/ONETRIANGLESIZE) + (2.0f*SIZEEEE_2*(z+SIZEEEE_2)/ONETRIANGLESIZE) ) + 1,
					(unsigned int)( ((x+SIZEEEE_2)/ONETRIANGLESIZE) + ((2.0f*SIZEEEE_2)*(z+SIZEEEE_2+ONETRIANGLESIZE)/ONETRIANGLESIZE) ) + 1
					);
				VBO(
					(unsigned int)( ((x+SIZEEEE_2)/ONETRIANGLESIZE) + (2.0f*SIZEEEE_2*(z+SIZEEEE_2)/ONETRIANGLESIZE) ),
					(unsigned int)( ((x+SIZEEEE_2)/ONETRIANGLESIZE) + ((2.0f*SIZEEEE_2)*(z+SIZEEEE_2+ONETRIANGLESIZE)/ONETRIANGLESIZE) ),
					(unsigned int)( ((x+SIZEEEE_2)/ONETRIANGLESIZE) + ((2.0f*SIZEEEE_2)*(z+SIZEEEE_2+ONETRIANGLESIZE)/ONETRIANGLESIZE) ) + 1
					);
