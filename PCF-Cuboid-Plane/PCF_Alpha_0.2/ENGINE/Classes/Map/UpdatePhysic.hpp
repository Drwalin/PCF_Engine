...
void Map::UpdatePhysic( unsigned int frametime )
{
    ...
    for( calculate = 1; calculate < objectphys.size(); calculate ++ )
    {
        for( a=0; a<2; a++){
        for( b=0; b<2; b++){
        for( c=0; c<2; c++){
    ...
    for( calculate = 1; calculate < object.size(); calculate ++ )
    {
        ...
        OBJNUM.collisionup = false;
        OBJNUM.collisiondown = false;
        // grawitacja:
    ...
    for( calculate = 0; calculate < object.size(); calculate ++ )
    {
        ...
        OBJNUM.pos += ( OBJNUM.speed * (float(frametime)/1000.0f) );
        
        CollisionAliveTriangle( calculate, frametime );
    }
