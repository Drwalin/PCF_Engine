...
bool CollisionObjectTriangle( unsigned int calculate, float x, float z, unsigned int time )
{
    ...
    for( unsigned int a = 0; a < trigger[int(x/XSIZETRIGGER)][int(z/ZSIZETRIGGER)].tri.size(); a++ )
    {
        ...
        {
            if( sqrt( pow(objnum.pos[0]-map.tri[trg].sphere[0][0],2)+pow(objnum.pos[1]-map.tri[trg].sphere[0][1],2)+pow(objnum.pos[2]-map.tri[trg].sphere[0][2],2) ) <= abs( objnum.radian/1.0f + map.tri[trg].radian[0] ) )
            {
            if( sqrt( pow(objnum.pos[0]-map.tri[trg].sphere[1][0],2)+pow(objnum.pos[1]-map.tri[trg].sphere[1][1],2)+pow(objnum.pos[2]-map.tri[trg].sphere[1][2],2) ) <= abs( objnum.radian/1.0f + map.tri[trg].radian[1] ) )
            {
            if( sqrt( pow(objnum.pos[0]-map.tri[trg].sphere[2][0],2)+pow(objnum.pos[1]-map.tri[trg].sphere[2][1],2)+pow(objnum.pos[2]-map.tri[trg].sphere[2][2],2) ) <= abs( objnum.radian/1.0f + map.tri[trg].radian[2] ) )
            {
                objnum.collision = true;
                    if( drad != 0.0f && rnormal != 0.0f )
                    {
    			        float pobjmove[3] = { ( map.tri[trg].normal[0] * ((0.99f*objnum.radian*drad/abs(drad))-drad) / rnormal ) , ( map.tri[trg].normal[1] * ((0.99f*objnum.radian*drad/abs(drad))-drad) / rnormal ) , ( map.tri[trg].normal[2] * ((0.99f*objnum.radian*drad/abs(drad))-drad) / rnormal ) };
