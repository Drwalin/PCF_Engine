...
inline void Player::PlayerUpdate( float time )
{
	if( object != NULL )
	{
		...
		{
			if( GameInstance -> engine.GetDynamicLightSourceHandle( 0 ) -> density > 0.5f )
				GameInstance -> engine.GetDynamicLightSourceHandle( 0 ) -> density = 0.0f;
			else
				GameInstance -> engine.GetDynamicLightSourceHandle( 0 ) -> density = 1.0f;
		}
		...
		GameInstance -> engine.GetDynamicLightSourceHandle( 0 ) -> origin = object->GetEyePos();
	}
}
