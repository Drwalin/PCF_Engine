...
inline void Player::UnablePhysicObject()
{
	object -> rayType.resize( 0 );
	object -> TerrainCollisionUse = false;
	object -> RayCollisionUse = false;
	object -> ObjectsCollisionUse = false;
	object -> Draw = false;
	object -> MoveUse = false;
	object -> model = NULL;
}
...
inline void Player::Delete()
{
	...
	weaponEquiped = 0;
	shooting = false;
	rotateDestiny.Set( 0.0f, 0.0f, 0.0f );
	rotateVelocity.Set( 0.0f, 0.0f, 0.0f );
	rotatePlus.Set( 0.0f, 0.0f, 0.0f );
}
