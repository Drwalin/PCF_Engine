...
inline int Game::LoadMap( char * srcFile )
{
	...
	if( file.good() )
	{
		...
		while( !file.eof() )
		{
			...
			else if( !strcmp( str, "lightSource" ) )
			{
				Vector origin;
				Vector direction;
				float range;
				...
				engine.AddLightSource( origin, direction, range, angle*0.5f, density );
			}
			else if( !strcmp( str, "object" ) )
			{
				...
				while( arg[0] != '}' )
				{
					...
					{
						file >> argi;
						if( argi >= 0 && argi < GameInstance->weapon.size() )
					...
					else if( !strcmp( arg, "weaponEquiped" ) )
					{
						...
						object.back() -> player -> weaponEquiped %= GameInstance->weapon.size();
					}
