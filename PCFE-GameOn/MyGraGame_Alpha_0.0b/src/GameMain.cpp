...
int main( int argc, char *argv[] )
{
	...
	// Load textures:
	game.engine.AddTexture( "..\\Media\\Textures\\textmodel.png",		GL_NEAREST );
	...
	game.engine.AddVBO( game.engine.TextureId( "Flower1" ) );
	...
	StaticGraphicModel * pointer = game.engine.GetStaticModelHandle( "Flower1" );
	...
	for( x = minx; x < maxx; x += size*0.05f )
	{
		for( z = minz; z < maxz; z += size*0.05f )
		{
			if( rand() < mingen && hmap3.GetHeight( x, z ) > 0.0f )
			{
				...
				game.engine.AddGraphicObject( pointer, pos, SetVector( 0.0f, rotatex, 0.0f ), scale );
