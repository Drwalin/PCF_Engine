...
void GUIFunction( float FrameTime )
{
	...
	
	{
		...
		engineInstantiation->SegmentCollision(
							engineInstantiation->Object(1)->pos+(engineInstantiation->Object(1)->size*SetVector(0.5f,0.8f,0.5f)),
							engineInstantiation->Object(1)->pos+(engineInstantiation->Object(1)->size*SetVector(0.5f,0.8f,0.5f)) + matrix.GetVertex( SetVector( 0.0f, 0.0f, 300.0f ) ),
							&object, &triangle, engineInstantiation->Object(1), SIGHT );
		glDepthFunc( GL_LESS );
		glLineWidth( 5.0f );
		glBegin( GL_LINES );
			glColor3f( 1.0f, 0.1f, 0.1f );
			glVertex3fv( &( ( engineInstantiation->Object(1)->pos+(engineInstantiation->Object(1)->size*SetVector(0.5f,0.8f,0.5f)) )[0] ) );
			glVertex3fv( &( ( engineInstantiation->Object(1)->pos+(engineInstantiation->Object(1)->size*SetVector(0.5f,0.8f,0.5f)) + matrix.GetVertex( SetVector( 0.0f, 0.0f, 300.0f ) ) )[0] ) );
		glEnd();
		*/
		matrix.InitEmpty();
		...
		engineInstantiation->SegmentCollision(
							engineInstantiation->Object(0)->pos+(engineInstantiation->Object(0)->size*SetVector(0.5f,0.9f,0.5f)),
							engineInstantiation->Object(0)->pos+(engineInstantiation->Object(0)->size*SetVector(0.5f,0.9f,0.5f)) + matrix.GetVertex( SetVector( 0.0f, 0.0f, 300.0f ) ),
							&object, &triangle, engineInstantiation->map.staticobject[0], SIGHT );
...
//	sprintf( FPS[1], "Physic FPS:  %f", timefps[2] );
    ...
    glEnable( GL_TEXTURE_2D );
		...
		{
			engineInstantiation->Object(0)->see[0] -= float(point.x-(glutGet(GLUT_WINDOW_X)+(glutGet( GLUT_WINDOW_WIDTH ) / 2))) * GameInstantiation->SensitivityX / (1000.0f);
			engineInstantiation->Object(0)->see[1] -= float(point.y-(glutGet(GLUT_WINDOW_Y)+(glutGet( GLUT_WINDOW_HEIGHT ) / 2))) * GameInstantiation->SensitivityY / (1000.0f);
        ...
        {
			engineInstantiation->map.particles.AddParticle(  SetVector( (engineInstantiation->Object(0)->pos[0]+(engineInstantiation->Object(0)->size[0]*0.5f)), (engineInstantiation->Object(0)->pos[1]+(engineInstantiation->Object(0)->size[1]*0.9f)), (engineInstantiation->Object(0)->pos[2]+(engineInstantiation->Object(0)->size[2]*0.5f)) ),
		...
		if( keyboardobiect.keypressed[GameInstantiation->SecondaryAttack] )
		{
			engineInstantiation->AddLightning(
						SetVector( (engineInstantiation->Object(0)->pos[0]+(engineInstantiation->Object(0)->size[0]*0.5f)), (engineInstantiation->Object(0)->pos[1]+(engineInstantiation->Object(0)->size[1]*0.9f)), (engineInstantiation->Object(0)->pos[2]+(engineInstantiation->Object(0)->size[2]*0.5f)) ),
						SetVector( (engineInstantiation->Object(0)->pos[0]+(engineInstantiation->Object(0)->size[0]*0.5f)), (engineInstantiation->Object(0)->pos[1]+(engineInstantiation->Object(0)->size[1]*0.9f)), (engineInstantiation->Object(0)->pos[2]+(engineInstantiation->Object(0)->size[2]*0.5f)) ) + ( matrix.GetVertex(forward) * ((float((rand()%10000)-5000)/100.0f)+70.0f) ),
						25.0f, 1.0f,
						0.4f, 0.4f, 1.0f,
						0.8f, 0.8f, 1.0f,
						10,
