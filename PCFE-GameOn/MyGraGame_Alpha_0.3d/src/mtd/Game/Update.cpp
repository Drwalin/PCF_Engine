...
inline void Game::Renderer( float time )
{
	...
	*/
	...
	
	{
		...
		matrix.InitEmpty();
		matrix.RotateY( time*4.0f );
		GameInstance->engine.map.lightSourcesDynamic[1] -> direction = matrix.GetVertex( GameInstance->engine.map.lightSourcesDynamic[1] -> direction );
