...
inline void Player::PlayerUpdate( float time )
{
	if( object != NULL )
	{
		...
		{
			if( GameInstance -> engine.GetDynamicLightSourceHandle( 2 ) -> density > 0.1f )
				GameInstance -> engine.GetDynamicLightSourceHandle( 2 ) -> density = 0.0f;
			else
				GameInstance -> engine.GetDynamicLightSourceHandle( 2 ) -> density = 1.0f;
		}
		
		matrix.InitEmpty();
		matrix.RotateY( object->see[0] + 90.0f );
		matrix.RotateX( -object->see[1] );
		GameInstance -> engine.GetDynamicLightSourceHandle( 2 ) -> direction = matrix.GetVertex( SetVector( 0.0f, 0.0f, 1.0f ) );
		GameInstance -> engine.GetDynamicLightSourceHandle( 2 ) -> origin = object->GetBarycentricDraw();
	}
}
