...
void GenerateMap___New()
{
	...
	float x, z;
	float minx = -100.0f, minz = -100.0f, maxx = 100.0f, maxz = 100.0f;
	...
	*/
	...
	for( x = minx; x < maxx; x += size )
	{
		for( z = minz; z < maxz; z += size )
		{
			...
			GameInstantiation->engine.AddVertexToLastVBO( x,      getheight(x,z), z,					(x)*0.01f,		(z)*0.01f );
			GameInstantiation->engine.AddVertexToLastVBO( x+size, getheight(x+size,z), z,				(x+size)*0.01f,	(z)*0.01f );
			GameInstantiation->engine.AddVertexToLastVBO( x+size, getheight(x+size,z+size), z+size,	(x+size)*0.01f,	(z+size)*0.01f );
			GameInstantiation->engine.CreateTriangleWithLastThreeVertices();
			
			GameInstantiation->engine.AddVertexToLastVBO( x,      getheight(x,z), z,					(x)*0.01f,		(z)*0.01f );
			GameInstantiation->engine.AddVertexToLastVBO( x,      getheight(x,z+size), z+size,		(x)*0.01f,		(z+size)*0.01f );
			GameInstantiation->engine.AddVertexToLastVBO( x+size, getheight(x+size,z+size), z+size,	(x+size)*0.01f,	(z+size)*0.01f );
			GameInstantiation->engine.CreateTriangleWithLastThreeVertices();
